...
class ClientsCard extends Component {
  renderClientsCardTable() {
    const clientsCard = this.props.clientsCard;

    return clientsCard.map((client, index) => (
      <tr key={index}>
...
export default withTracker((props) => {
  const clientsCardsSubscriptions = Meteor.subscribe('ClientsCardCollection');
  const loading = clientsCardsSubscriptions ? !clientsCardsSubscriptions : true;

  return {
    loading,
    clientsCard: ClientsCardCollection.find().fetch(),
  };
})(ClientsCard);
